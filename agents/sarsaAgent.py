...
class SARSAAgent(AbstractAgent):
    ...
    def __init__(self, observation_space, action_shape,
        ...
        # Choosing next action with policy in update(), need to save it to perform it in next step
        self.next_action = None


        
    def get_action(self, state):
        # We choose the next action in the previous update
        if self.next_action:
            return self.next_action

        # We only need to choose the first action here.
        # In eval mode, next action will never be set, so it chooses here the next action then. 
    ...
    def get_policy_action(self, state):
        ...
        # Action selection with Epsilon-Greedy (our policy)
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        if done:
            q_next = 0
            self.next_action = None # Important to reset next action for next episode
        else:
            # NOTE: SARSA
            # Choose next action before
            self.next_action = self.get_policy_action(next_state)
            # Using the Q-value of the next choosen action (on-policy) 
            q_next = self.q_table[str(next_state)][self.next_action]
